...
SNAPSHOT_USAGE_WARN_PCT=20            # Percent of total pool capacity to start suggesting action
SNAPSHOT_USAGE_WARNING_PCT=65         # Percent triggers WARNING status
SNAPSHOT_USAGE_CRITICAL_PCT=90        # Percent triggers CRITICAL (unless suppressed logically)
FORCE_PRUNE_FREE_GB=300               # If free space (aggregate) > this, logical overages may be suppressed

# === Retention ===
# Minimum & default per RAID profile (higher RAID redundancy => keep more)
MIN_SNAPSHOTS_KEEP=1                  # Never prune below this count per share
DEFAULT_KEEP_RAID1=3                  # Keep last N snapshots per share on RAID1
DEFAULT_KEEP_SINGLE=2                 # Keep last N snapshots per share on single-profile pools
DEFAULT_KEEP_RAID0=2                  # Keep last N snapshots per share on RAID0

# === Behavior Toggles ===
STOP_DOCKER_VMS=false                 # true=stop Docker & libvirt during snapshot creation
SUPPRESS_LOGICAL_WARNINGS=true        # true=prefer physical accounting; suppress logical overage unless low free space

# === Logging & Performance ===
LOG_LEVEL="INFO"                      # INFO or DEBUG (adds per-snapshot method lines)
SEND_THROTTLE="200m"                  # Throttle btrfs send size estimation via pv (empty disables)
...
# === Helpers Functions ===
# Best-effort physical bytes used by a snapshot path
...
# === Helpers Functions ===
# Estimate send-stream size between parent and snapshot
estimate_send_size() {
...
for POOL in "${POOLS[@]}"; do
  ...
  RAID_TYPE=$(btrfs filesystem df "$POOL" 2>/dev/null | grep -m1 "Data," | awk -F',' '{print $2}' | awk '{print tolower($1)}')
  RAID_TYPE=${RAID_TYPE%:}
  ...
  # Adjust retention based on free space
  ...
  for SHARE in "${SHARES[@]}"; do
    ...
    if [ "$num_snaps" -gt "$SNAPSHOTS_TO_KEEP" ]; then
      ...
      # Sort snapshot entries by mtime using mapfile for safer array population
  ...
  for SHARE in "${SHARES[@]}"; do
    ...
    for snap in "${share_snaps[@]:-}"; do
      ...
      if printf '%s' "$phys" | grep -Eq '^[0-9]+$' && [ "$phys" -gt 0 ]; then
        ...
        log "$snap method=filesystem_du bytes=$phys"
      else
        if [ "$allow_estimate" = true ]; then
          ...
          if printf '%s' "$est" | grep -Eq '^[0-9]+$' && [ "$est" -gt 0 ]; then
            ...
            log "$snap method=send_estimate parent=$(basename "$parent") bytes=$est"
            j=$((j+1))
            continue
          fi
        fi
...
log "Summary: $notify_subject - $notify_short"
